...
utterances_slim <- utterances %>% select(utterance_id, session_id_fk, utterance_id_source, corpus, language, speaker_label, addressee, childdirected, utterance, translation, utterance_morphemes, start, end, comment, warning)
...
sessions_slim <- sessions %>% select(session_id, session_id_source, date)
speakers_slim <- speakers %>% select(speaker_id, session_id_fk, uniquespeaker_id_fk, speaker_label, name, age_raw, age, age_in_days, gender_raw, gender, role_raw, role, macrorole, languages_spoken, birthdate)
...
left_join(u_w_m_s, speakers_slim, by=c("session_id"="session_id_fk", "speaker_label"="speaker_label")) -> all_data
# left_join(u_w_m_s, speakers_slim, by=c("speaker_id_fk"="speaker_id")) -> all_data # future way to merge
...
all_data <- all_data %>% select(corpus, language, session_id, session_id_source, date, speaker_id, speaker_label, name, birthdate, age_raw, age, age_in_days, gender_raw, gender, role_raw, role, macrorole, addressee, childdirected, utterance_id, utterance_id_source, start, end, utterance, translation, utterance_morphemes, comment, word_id, word, word_actual, word_target, pos_word_stem, morpheme_id, morpheme_type, morpheme, gloss_raw, gloss, pos_raw, pos, morpheme_language)
