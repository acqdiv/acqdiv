...
# TODOs: refactor this module
# Importing this module in other modules is irksome because
# - no classes (-> test cases cannot only be loaded by nose but not unittest)
# - loading of database is done globally

cwd_path = os.path.dirname(__file__)

# connect to the test db
test_db_path = os.path.join(cwd_path, "../database/test.sqlite3")
test_db = sqlite3.connect(test_db_path)

# if the sqlite3 gold database exists
gold_db_path = os.path.join(cwd_path, "fixtures/gold.sqlite3")
if os.path.isfile(gold_db_path):
    # delete it first
    os.remove(gold_db_path)

# generate a fresh sqlite3 gold database from the dump
dump_path = os.path.join(cwd_path, "fixtures/gold.dump")
command = "sqlite3 {} < {}".format(gold_db_path, dump_path)
subprocess.run(command, shell=True)

# connect to it
gold_db = sqlite3.connect(gold_db_path)
